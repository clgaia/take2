...
def GenerateUI():
    ...
    root.geometry(
        f'{window_width}x{window_height}+{center_x}+{center_y}')  # set the position of the window to the center of the screen
    ...
    def updateConnection(connectionStatus, connectionLabel, gif, prevStatus):
        if os.path.exists('/home/pi/Desktop/MuseAlarm/data/connection.csv'):  # arr
            try:
                connection = pd.read_csv('/home/pi/Desktop/MuseAlarm/data/connection.csv').iloc[0, 1]
                ...
                print("Exception using Pandas and iloc occured", traceback.print_exc())
